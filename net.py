...
class ConditionalInitialization(nn.Module):

    def __init__(self, embed_dim, param_size, param_shape):
        super(ConditionalInitialization, self).__init__()
        self.num_channels = 3
        self.embed_dim = embed_dim
        self.layer_size = param_size
        self.param_shape = param_shape

        self.c_encoder = nn.Sequential(*[
            nn.Linear(self.embed_dim, 10),
            nn.ReLU(),
            nn.Linear(10, param_size)
        ])

    def forward(self, inputs):
        params_l = self.c_encoder(inputs)

        params = params_l.view(self.param_shape)
